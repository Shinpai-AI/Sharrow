...
def main():
    ...
    parser.add_argument("--rules-dir", type=Path, default=RULES_DEFAULT)
    parser.add_argument("--models-dir", type=Path, default=MODELS_DEFAULT)
    parser.add_argument("--reports-dir", type=Path, default=REPORTS_DEFAULT)
    ...
    symbols_cfg = config.get("symbols", {})
    symbols = [sym for sym in symbols_cfg.keys() if len(sym) >= 3]
    if not symbols:
    ...
    write_summary(results, args.reports_dir)
