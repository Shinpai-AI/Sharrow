#!/bin/bash
# RUN-hourly-news.sh v6.2
# Goldjunge Hourly News Workflow – Pfade kommen automatisch aus TKB-config.json
...
NEWS_DIR="$SCRIPT_DIR"
LOG_FILE="$SCRIPT_DIR/hourly-news.log"
...
log "🚀 Goldjunge Hourly News Workflow gestartet"
...
log "=== Workflow beendet ==="
log "📄 Erfolgreich kopiert: $FILES_COPIED"
log "❌ Fehler: $FILES_FAILED"

if [ $FILES_COPIED -gt 0 ] && [ $FILES_FAILED -eq 0 ]; then
    echo "✅ GOLDJUNGE Hourly News Workflow SUCCESSFUL!"
    exit 0
else
    echo "❌ GOLDJUNGE Hourly News Workflow FAILED!"
    exit 1
fi
